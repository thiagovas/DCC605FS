...
// Set the superblock
void initfs_superblock(int fd, struct superblock *sb)
{
	int ret;
	ret=write(fd, (void*)sb->magic, sizeof(uint64_t));
	ret=write(fd, (void*)sb->blks, sizeof(uint64_t));
	ret=write(fd, (void*)sb->blksz, sizeof(uint64_t));
	ret=write(fd, (void*)sb->freeblks, sizeof(uint64_t));
	ret=write(fd, (void*)sb->freelist, sizeof(uint64_t));
	ret=write(fd, (void*)sb->root, sizeof(uint64_t));
	ret=write(fd, (void*)sb->fd, sizeof(uint64_t));
}
